function exportLocalsPdf(localSpeakers) {
  ...
  doc.setFontSize(9);
  doc.text(`Generado el ${today.toLocaleDateString("es-ES")} \xB7 pr\xF3ximos 12 meses`, marginX, y);
  y += 24;
  ...
  sorted.forEach((sp) => {
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.text("Meses disponibles (pr\xF3ximo a\xF1o):", marginX, y);
    y += 13;
    const blocked = new Set(sp.blockedMonths || []);
    const line = next12.map((m) => `${m.label}${blocked.has(m.key) ? " (ocupado)" : ""}`).join("   \xB7   ");
