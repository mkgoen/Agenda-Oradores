...
function speakerEvents(speaker, events) {
  return events.filter((e) => e.speakerId === speaker.id);
}
function nextUpcomingEvent(evs) {
  const t = todayMidnight();
  const upcoming = evs.filter((e) => e.date && /* @__PURE__ */ new Date(e.date + "T00:00:00") >= t).sort((a, b) => a.date.localeCompare(b.date));
  return upcoming[0] || null;
}
function lastPastEvent(evs) {
  const t = todayMidnight();
  const past = evs.filter((e) => e.date && /* @__PURE__ */ new Date(e.date + "T00:00:00") < t).sort((a, b) => b.date.localeCompare(a.date));
  return past[0] || null;
}
...
const SORT_OPTIONS = [
  { key: "alpha", label: "Alfab\xE9tico" },
  { key: "place", label: "Lugar" },
  { key: "next", label: "Pr\xF3ximo evento" },
...
function groupAndSortSpeakers(list, events, mode, dir) {
  ...
  if (mode === "next") {
    ...
    return {
      primary: withNext.map((x) => x.s),
      primaryLabel: "Pr\xF3ximo evento",
      secondary: withoutNext.map((x) => x.s),
      secondaryLabel: "Sin eventos pr\xF3ximos (pasados)"
    };
  }
  if (mode === "invitados" || mode === "previstos") {
    ...
    return {
      primary: withStatus.map((x) => x.s),
      primaryLabel: status,
      secondary: rest,
      secondaryLabel: "Otros estados"
    };
  }
