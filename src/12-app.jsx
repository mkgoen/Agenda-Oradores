...
/* ---------------------------------------------------------------
   Reconciliación de "Meses libres"
   -----------------------------------------------------------------
   Versiones anteriores de la app solo marcaban el mes como ocupado
   para las "salidas" (más tarde se extendió a visitas y eventos).
   Esto dejó discrepancias acumuladas en copias de seguridad antiguas:
   oradores locales con un evento registrado en un mes que NO aparece
   en sus "meses libres". Esta función corrige eso: por cada orador
   local, añade a blockedMonths el mes de cada evento que tenga
   asociado. Solo AÑADE meses, nunca quita ninguno — así no se borra
   ninguna preferencia manual que no tenga un evento detrás.

   Esta distinción (mes bloqueado con evento real vs. bloqueado a
   mano sin evento) es justo la que luego se colorea en la ficha del
   orador: rojo para los que sí tienen un evento en la agenda, naranja
   para los puramente manuales (ver 08-speakers-view.jsx).
----------------------------------------------------------------*/
function reconcileBlockedMonths(speakers, events) {
  return speakers.map(sp => {
    ...
    if (months.size === before) return sp; // sin discrepancias, no se toca
...
function App() {
  ...
  // Base de datos de títulos por número de bosquejo (1-194), usada para
  // autocompletar el título en eventos e invitaciones.
  ...
  const handleOpenFile = () => {
    openAgendaFile()
      .then(d => {
        ...
        setTimeout(() => setFileMessage(""), fixedCount > 0 ? 5000 : 2500);
      })
      .catch(e => {
  ...
  const saveEvent = (form) => {
    ...
    // Cualquier evento (visita, salida o evento) en el que participe un
    // orador local marca ese mes como no disponible automáticamente.
    ...
    // Si se edita un evento y cambia el orador o el mes, revisamos si el
    // orador anterior sigue teniendo algo más ese mes; si no, se libera.
    if (modal.initial) {
  ...
  return (
    <div className="h-screen flex flex-col" style={{ background: COLORS.bg, fontFamily: "Inter, sans-serif", color: COLORS.ink }}>
      <style>{`
        ...
        /* Zoom ligero + resplandor inferior al pasar el ratón, reutilizado
           en pestañas, celdas del calendario, tarjetas de orador y el
           navegador de años. */
      ...
      <main className="px-6 py-5 flex-1 overflow-y-auto min-h-0">
        {!loaded ? (
          ...
          <BosquejosView events={events} raw={bosquejosRaw} setRaw={setBosquejosRaw} unavailable={unavailableBosquejos} setUnavailable={setUnavailableBosquejos}
            bosquejoTitles={bosquejoTitles} setBosquejoTitles={setBosquejoTitles}
            dbExpanded={bosquejoDbExpanded} setDbExpanded={setBosquejoDbExpanded} />
        ) : (
          <YearView
      ...
      {modal && (
        <EventModal
          ...
          speakers={speakers} events={events} statuses={statuses} unavailableBosquejos={unavailableBosquejos} bosquejoTitles={bosquejoTitles}
