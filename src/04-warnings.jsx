...
function computeWarnings(form, events, speakers, excludeId, unavailableBosquejos) {
  ...
  if (form.type === "visita" && form.speechNumber && form.date) {
    ...
    if (dup) {
      warnings.push({ key: "duplicate", text: `El discurso nº ${form.speechNumber} ya se dio aquí el ${formatDate(dup.date)}, con ${dup.speakerName}. Han pasado menos de 12 meses.` });
    }
  }
  if (form.speechNumber && unavailableBosquejos && unavailableBosquejos.includes(String(form.speechNumber).trim())) {
    warnings.push({ key: "unavailableBosquejo", text: `El bosquejo nº ${form.speechNumber} está marcado como no disponible.` });
  }
  if (form.type === "salida" && form.speakerName && form.date) {
    const period = ym(form.date);
    const count = events.filter(ev =>
      ev.id !== excludeId && ev.type === "salida" &&
      ev.speakerName?.trim().toLowerCase() === form.speakerName.trim().toLowerCase() &&
      ym(ev.date) === period
    ).length;
    if (count >= 1) {
    ...
    if (speaker?.blockedMonths?.includes(period)) {
      ...
      const selfCausedBlock = original && original.type === "salida" &&
        original.speakerName?.trim().toLowerCase() === form.speakerName.trim().toLowerCase() &&
        ym(original.date) === period;
      if (!selfCausedBlock) {
        warnings.push({ key: "blocked", text: `${form.speakerName} pidió no ser invitado durante ${MONTHS[Number(period.slice(5,7)) - 1]} de ${period.slice(0,4)}.` });
      }
    }
  }
  return warnings;
